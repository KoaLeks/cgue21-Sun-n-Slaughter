...
std::shared_ptr<Node> Scene::processNode(aiNode* node, const aiScene* scene, int level, bool transformation,
	...
	if (!tmpnam.compare(0, enemyPrefix.size(), enemyPrefix)) {
		isEnemy = true;
		newNode = std::make_shared<Enemy>(highscore, _soundEngine);
	}
	else {
...
void Character::move2(glm::vec3 dir, float speed, float dt) {
...
void Character::animate(int step, int _order[4]) {
	glUseProgram(_animationShader);
	...
	if (_order != nullptr) {
		input = _order[(step / 5) % 4];
		output = _order[((step / 5) + 1) % 4];
	}
