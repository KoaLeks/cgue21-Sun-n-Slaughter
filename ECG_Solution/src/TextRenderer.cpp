...
TextRenderer::TextRenderer(GLuint width, GLuint height)
{
	...
	TextShader->use();
	TextShader->setUniform("projection", glm::ortho(0.0f, static_cast<GLfloat>(width), static_cast<GLfloat>(height), 0.0f));
...
void TextRenderer::RenderText(std::string text, GLfloat x, GLfloat y, GLfloat scale, glm::vec3 color)
{
	...
	for (c = text.begin(); c != text.end(); c++)
	{
		...
		GLfloat xpos = x + 0.5f + ch.Bearing.x * scale;
		GLfloat ypos = y + (this->Characters['P'].Bearing.y - ch.Bearing.y) * scale;
...
void TextRenderer::RenderImage(GLuint imagePos, GLfloat x, GLfloat y, GLfloat scale, glm::vec3 color) {
	...
	// Render glyph texture over quad
	glBindTexture(GL_TEXTURE_2D, imagePos);
	// Update content of VBO memory
	glBindBuffer(GL_ARRAY_BUFFER, VBO);
	glBufferSubData(GL_ARRAY_BUFFER, 0, sizeof(vertices), vertices); // Be sure to use glBufferSubData and not glBufferData
